...
namespace IASLib
{
    ...
    template <class T>
    class TListElement : public CObject
    {
    public:
        DECLARE_OBJECT(TListElement<T>, CObject)

    private:
        T *data;
        TListElement<T> *pNext;
        TListElement<T> *pPrev;

    public:
        TListElement<T>(T *value)
        {
            data = value;
            pNext = nullptr;
            pPrev = nullptr;
        ...
        TListElement<T>(T *value, TListElement<T> *before, TListElement<T> *after)
        {
            data = value;
            if (before != nullptr)
            {
    ...
    private:
        TListElement *head;
        TListElement *tail;
#ifdef __IASLIB_THREAD_SAFE
        CMutex *mutex;
#endif

    public:
        DECLARE_OBJECT(TDoubleList<T>, TCollection<T>)
    ...
    public
        ...
        {
            tail = new TListElement<T>(element, tail, nullptr);
            return true;
        }
